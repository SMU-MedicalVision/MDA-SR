...
from utils.filters import find_kernel
from loss.data_loss import get_data_loss
...
class AdaptiveDownsamplingModel(nn.Module):
    ...
    def find_kernel(self):
        return find_kernel(self.img_s[0], self.img_gen[0], scale=self.args.scale, k=self.args.adl_ksize, max_patches=-1)
    ...
    def resume(self, model_dir, train=True):
        ...
        if train:
            ...
            if self.data_loss_type == 'adl' and (checkpoint['ep'] + 1 > self.args.adl_interval):
                ...
                self.down_filter = self.down_filter.cuda(self.gpu)

        return 0, 0 #checkpoint['ep'], checkpoint['total_it']
